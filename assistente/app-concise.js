(() => {
  ...
  const state = { salesAgent:null, lastTopic:null, lastVariant:Object.create(null), busy:false };
  ...
  function looksLikeStoreOffering(text){
    if(hasAny(text, storeSellingPhrases)) return true;
    const startsWithAvailability = /^(tem|tem alguma|tem algum|voces tem|vcs tem|vende|vendem|voces vendem|vcs vendem)\b/.test(text);
    const mentionsProduct = hasJewelry(text) || hasAny(text, readyProducts) || hasAny(text, semijewelryWords);
    const firstPersonPossession = hasAny(text, ["tenho","minha","meu","minhas","meus","possuo"]);
    return startsWithAvailability && mentionsProduct && !firstPersonPossession;
  }

  function looksLikeClientSelling(text){
    if(hasAny(text, clientSellingStrong)) return true;
    const sellVerb = hasExactWord(text, ["vender","avaliar","vendo"]);
    const product = hasJewelry(text) || hasAny(text, ["ouro","prata"]);
    const firstPerson = hasAny(text, ["quero","queria","gostaria","preciso","tenho","minha","meu","minhas","meus","possuo"]);
    return sellVerb && product && firstPerson;
  }

  function isSellingAmbiguous(text){
    const product = hasJewelry(text) || hasAny(text, ["ouro","prata"]);
    const sellLanguage = hasAny(text, ["para vender","pra vender","vender"]);
  ...
  function classify(raw){
    ...
    if(state.lastTopic === "sale_direction_clarify"){
      if(hasAny(text, ["comprar","ver joias","loja","quero uma","quero comprar","voces vendem"])) return "store_products";
      if(hasAny(text, ["vender minhas","vender meu","vender minha","avaliar","tenho para vender","quero vender"])) return "sell_gold_silver";
    }
    ...
    if(state.lastTopic === "gold_price_clarify"){
      if(looksLikeClientSelling(text) || hasAny(text, ["vender","avaliar","meu ouro","minha prata"])) return "sell_gold_silver";
      if(hasAny(text, ["comprar","joia","alianca","anel","corrente"])) return "commercial";
    }
    if(state.lastTopic === "personalized" && (hasJewelry(text) || hasAny(text, ["ouro","prata","foto","desenho"]))) return "personalized_commercial";
    ...
    if(looksLikeStoreOffering(text)){
      ...
      if(hasAny(text, semijewelryWords)) return "store_semijewelry";
      if(hasAny(text, ["corrente de ouro","correntes de ouro"])) return "store_gold_chains";
    ...
    const ringIntent = hasRing(text) && (hasAny(text, desireWords) || hasAny(text, priceWords) || hasAny(text, stockWords) || hasAny(text, ringOccasions) || hasAny(text, ["modelo","modelos","ouro","prata","largura","tamanho","gramatura","milimetragem"]));
  ...
  const R = Object.freeze({
    ...
    store_products:["Temos joias à venda, sim. Os modelos e valores atuais estão no site.","Sim. Você pode conferir as peças disponíveis na loja oficial.","Temos opções à pronta entrega. Vou abrir a loja para você."],
    ...
    discount:["As promoções mudam. O vendedor verifica a melhor condição.","Pode haver um valor melhor que o site para alianças e encomendas.","Vou te encaminhar para consultar a promoção atual."],
    site_price_vs_service:["Para pronta entrega, vale o preço do site. Em anéis e alianças, o atendimento pode ter promoções.","O site mostra uma referência. A peça sob medida é calculada pelo vendedor.","Anéis e alianças podem ter condições diferentes do valor online."],
    order_tracking:["A equipe consegue consultar usando os dados do pedido.","Vou te encaminhar para verificar o rastreio.","Tenha em mãos o nome ou número do pedido."],
    ...
    gold_care:["O ouro 18k pode perder brilho por resíduos ou produtos químicos.","Às vezes é apenas acúmulo de resíduos. A equipe pode analisar.","Evite produtos caseiros fortes na peça."],
    ...
    semijewelry:["Temos semijoias e fazemos consertos quando a peça permite.","Os modelos disponíveis ficam no site.","Trabalhamos com semijoias, sim."],
    repair_clarify:["Qual peça precisa de conserto?","É uma joia ou semijoia?","Me diga qual é a peça."],
    repair:["Consertamos joias e semijoias, após análise da peça.","Envie uma foto para a equipe avaliar o reparo.","Fazemos esse conserto quando é tecnicamente possível."],
    ...
    sale_direction_clarify:["Você quer comprar joias ou vender suas peças?","É para comprar uma joia ou vender uma que você já tem?","Você procura joias à venda ou quer vender as suas?"],
    ...
    unsupported_material:["Trabalhamos somente com ouro e prata. Não fazemos peças de moeda ou outros metais.","Não confeccionamos nem compramos esse material.","Nossa produção é apenas em ouro 18k e prata 925."],
    commercial_clarify:["Qual tipo de joia você procura?","Você procura anel, corrente, pulseira, brinco ou outra peça?","Me diga qual peça você tem em mente."],
  ...
  const META = Object.freeze({
    rings_order:{action:"sales",label:"Ver modelos e valores"}, personalized_commercial:{action:"sales",label:"Enviar referência"},
    payment:{action:"sales",label:"Consultar pagamento"}, boleto_special:{action:"sales",label:"Simular boleto"},
    discount:{action:"sales",label:"Consultar promoção"}, site_price_vs_service:{action:"sales",label:"Confirmar valor"},
    order_tracking:{action:"sales",label:"Consultar pedido"}, commercial:{action:"sales",label:"Falar com atendimento"},
    repair:{action:"repair",label:"Enviar foto da peça"}, sell_gold_silver:{action:"evaluation",label:"Avaliar ouro ou prata"},
    store_stock:{store:"products",label:"Abrir loja oficial"}, store_products:{store:"products",label:"Ver produtos"},
    store_semijewelry:{store:"semijewelry",label:"Ver semijoias"}, store_gold_chains:{store:"goldChains",label:"Ver correntes de ouro"},
    plated_explanation:{store:"platedInfo",label:"Ver explicação completa"}, semijewelry_warranty:{store:"warranty",label:"Ver garantia"}
  });
  ...
  function pick(topic){
    ...
    let index;
    do { index = Math.floor(Math.random() * list.length); } while(index === state.lastVariant[topic]);
  ...
  function whatsappUrl(type, context){
    ...
    const lead = type === "repair" ? "Olá! Preciso avaliar o conserto de uma joia ou semijoia." : type === "evaluation" ? "Olá! Quero avaliar itens de ouro ou prata para venda." : "Olá! Vim pelo assistente Coroa 24K e quero atendimento.";
  ...
  async function handle(raw){
    ...
    const delay = Math.min(720, 240 + question.length * 5 + Math.floor(Math.random() * 100));
  ...
  if(new URLSearchParams(location.search).has("test")){
    const tests = [
      ["tem joias pra vender?","store_products"],["vocês têm joias para vender?","store_products"],["vocês vendem joias?","store_products"],["tem corrente para vender?","store_products"],
      ["tenho joias pra vender","sell_gold_silver"],["quero vender minhas joias","sell_gold_silver"],["tenho ouro para vender","sell_gold_silver"],["quanto vocês pagam no ouro?","sell_gold_silver"],
      ["joias pra vender","sale_direction_clarify"],["gostaria de uma aliança de casamento","rings_order"],["queria um par de alianças para casar","rings_order"],
      ["estou procurando uma alianca de noivado","rings_order"],["tenho interesse em um solitário","rings_order"],["gostaria de uma aliansa","rings_order"],
      ["queria uma corrente","store_products"],["procuro semijoias","store_semijewelry"],["gostaria de uma joia","commercial_clarify"],
      ["consertam corrente de prata","repair"],["consertam relógio","unsupported_repair"],["vocês fazem conserto","repair_clarify"],
      ["tem boleto","boleto_special"],["qual o valor do ouro","gold_price_clarify"],["fazem anel de moeda","unsupported_material"],
      ["o frete é gratis","shipping"],["a gravação é gratuita","engraving"],["qual a garantia da semijoia","semijewelry_warranty"],
      ["quanto custa uma pulseira","store_products"],["qual a diferença entre ouro e prata","material_comparison"]
    ];
    ...
    const tooLong = Object.entries(R).flatMap(([topic,list]) => list.filter((text) => text.replace(/<[^>]+>/g, "").length > 180).map((text) => ({topic,length:text.length,text})));
    const output = $("#test-results");
    output.style.display = "block";
    output.textContent = JSON.stringify({total:tests.length,passed:tests.length - failures.length,failures,tooLong}, null, 2);
