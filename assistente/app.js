(() => {
  ...
  function chooseSalesAgent(){
    ...
    try{
      ...
      state.salesAgent = CONFIG.sales[Math.random() < .5 ? 0 : 1];
    }
  ...
  function classify(raw){
    ...
    const repair = ["conserto","concerto","concertar","consertar","consertam","concertam","arrumar","reparar","reparo","soldar","solda","quebrou","quebrada","quebrado","apertar pedra","pedra caiu","polimento","polir","banho de novo","restaurar","ajustar aro","ajuste de aro","aumentar aro","diminuir aro","reformar joia"];
    ...
    const unsupported = ["aco","tungstenio","titanio","latao","em cobre","compram cobre","vender cobre","trabalham com cobre","em bronze","compram bronze","vender bronze","rodio","bijuteria","alianca de moeda","anel de moeda","feito de moeda","joia de moeda","compram moeda","vender moeda"];
    ...
    const purchaseTerms = ["quanto custa","qual valor","qual o valor","preco","orcamento","quanto fica","parcelamento","parcela","parcelam","desconto","promocao","comprar","quero uma","quero um","encomendar","fazer pedido","fazer um pedido","como pedir","catalogo","modelos disponiveis","tem disponivel","valor da alianca","valor das aliancas","valor do anel","quero alianca","quero aliancas","quero joia","quero joias","quero anel","ver aliancas","ver joias","falar com especialista","falar com atendente","falar com vendedor","quero falar com um especialista","quero falar com um atendente","vendem ouro","vendem prata","vendem alianca","vendem aliancas","vendem joia","vendem joias","tem alianca","tem aliancas","tem joia","tem joias","quantas gramas","quanto pesa","peso da alianca","peso das aliancas","peso do anel"];
    const stockTerms = ["estoque","em estoque","pronta entrega","pronto entrega","pronta pronta","disponivel agora","disponiveis agora","entrega imediata","comprar pronto","quais produtos tem","o que tem disponivel","ver produtos","loja online","loja oficial","site da loja","comprar pelo site","catalogo online"];
    const semijewelryTerms = ["semi joia","semijoia","semijoias","banhada","banhado","folheada","folheado"];
    const readyProductTerms = ["corrente","correntes","colar","colares","pulseira","pulseiras","brinco","brincos","pingente","pingentes","tornozeleira","tornozeleiras","piercing","piercings","escapulario","choker"];
    ...
    if(hasAny(t, personalization) && hasAny(t, purchaseTerms)) return "personalized_commercial";
    if(hasAny(t, personalization)) return "personalized";

    if(hasAny(t, ringTerms) && (hasAny(t, purchaseTerms) || hasAny(t, stockTerms) || hasAny(t,["modelos","modelo","disponibilidade","milimetros","milimetragem","gramatura","tamanho"]))) return "rings_order";
    if(hasAny(t, stockTerms) && hasAny(t, semijewelryTerms)) return "store_semijewelry";
    if(hasAny(t, stockTerms) && hasAny(t,["corrente de ouro","correntes de ouro","ouro 18k"])) return "store_gold_chains";
    if(hasAny(t, stockTerms)) return "store_stock";

    if(hasAny(t,["macica","macico","oca","oco"])) return "solid_or_hollow";
    if(hasAny(t,["largura","milimetros","3mm","4mm","5mm","6mm","alianca fina","alianca larga"])) return "width_style";
    ...
    if(hasAny(t,["pagamento","pix","cartao","boleto","entrada","pagar na entrega","pagamento na entrega","formas de pagamento"])) return "payment";
    ...
    if(hasAny(t,["ouro 24k","24 quilates","ouro puro"])) return "gold24k";
    ...
    if(hasAny(t,["garantia da semijoia","garantia semijoia","garantia do banho","garantia folheado","garantia banhado"])) return "semijewelry_warranty";
    ...
    if(hasAny(t,["garantia","certificado","nota fiscal","autenticidade","procedencia"])) return "trust_docs";
    ...
    if(hasAny(t,["limpar","limpeza","cuidar","cuidados","escureceu","ficou preta","oxidou","manutencao"])) return "care";
    if(hasAny(t,["diamante","pedra","zirconia","safira","ametista","esmeralda","rubi"])) return "stones";
    if(hasAny(t,["onde fica","onde ficam","onde voces ficam","endereco","loja fisica","curitiba","bairro alto","empresa confiavel","sao confiaveis","e confiavel","golpe","seguranca","avaliacoes google"])) return "location_trust";
    if(hasAny(t,["ouro 10k","10 quilates","ouro 14k","14 quilates"])) return "other_gold_karat";
    if(hasAny(t,["o que e semijoia","o que e uma semijoia","peca banhada","como funciona o banho","milesimos","verniz cataforetico"])) return "plated_explanation";

    if(hasAny(t, semijewelryTerms) && (hasAny(t, purchaseTerms) || hasAny(t, readyProductTerms))) return "store_semijewelry";
    if(hasAny(t,["corrente de ouro","correntes de ouro"]) && (hasAny(t, purchaseTerms) || hasAny(t,["modelos","estoque"]))) return "store_gold_chains";
    if(hasAny(t, readyProductTerms) && hasAny(t, purchaseTerms)) return "store_products";
    if(hasAny(t,["produtos","catalogo da loja","catalogo oficial","site","loja"]) && hasAny(t,["ver","abrir","conhecer","acessar","comprar"])) return "store_stock";

    if(hasAny(t, semijewelryTerms)) return "semijewelry";
    if(hasAny(t, ringTerms) && hasAny(t,["fazem","vendem","tem","trabalham com","quero","procuro"])) return "rings_order";
    if(hasAny(t,["joia","joias","pingente","corrente","pulseira"]) && hasAny(t, purchaseTerms)) return "commercial";
    if(hasAny(t, purchaseTerms)) return "commercial";
  ...
  const RESPONSES = {
    ...
    warranty_scope: { text:"A garantia permanente da linha de joias é sobre o <strong>teor do ouro 18k ou da prata 925</strong>. Quebras, riscos, pedras, deformações e desgastes dependem da causa e precisam de análise técnica para definir reparo e custo.", action:"repair", actionLabel:"Solicitar análise da garantia" },
    semijewelry: { text:"Também trabalhamos com semijoias banhadas e realizamos consertos quando a peça permite reparo. Para ver modelos e estoque atual, acesse a loja oficial. Para conserto, a equipe técnica analisa primeiro a peça.", quick:["Ver semijoias disponíveis","Preciso consertar semijoia","Como funciona o banho?"] },
    ...
    thanks: { text:"Por nada! Quando precisar, é só me perguntar. Posso ajudar com pronta entrega, alianças, personalizados, consertos ou avaliação de ouro e prata.", quick:["Ver pronta entrega","Quero alianças","Falar com atendente"] },
    unknown: { text:"Ainda não encontrei uma resposta segura para essa pergunta. Para não passar informação incompleta, você pode reformular, consultar a loja oficial para preços e estoque ou falar diretamente com um especialista.", quick:["Ver pronta entrega","Alianças e anéis","Vender ouro ou prata","Conserto","Falar com especialista"] }
  };
  ...
  function addAction(type, label, context){
    ...
    note.textContent = type === "sales" ? "Você será direcionado a um dos atendentes comerciais." :
      type === "repair" ? "O reparo depende da análise da peça." :
  ...
  async function handleQuestion(raw){
    ...
    state.busy = true;
    input.value = "";
    const safe = question.replace(/[&<>"']/g, (m) => ({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#039;"}[m]));
